...
-   Course location: 140.776.01 is in person in Sommer Hall (Wolfe E2014).
...
-   Lidio Jaimes Jr ([ljaimes1\@jh.edu](mailto:ljaimes1@jh.edu))
-   Bella Satpathy-Horton ([ghorton2\@jhmi.edu](mailto:ghorton2@jhmi.edu))

### Office hours

Instructor and TA office hours are [announced on CoursePlus](https://courseplus.jhu.edu/core/index.cfm/go/pb:page.view/coid/26074/pageID/235789/). If there are conflicts and/or need to cancel office hours, announcements will be made on CoursePlus.
...
There is no required textbook. We will make use of several freely available textbooks and other materials. All course materials will be provided. We will use the `R` software for data analysis, which is freely available for download. We will also use `Git` for code version control and create a `GitHub` account. `Git` is also free to download and `GitHub` provides free accounts. We will also use `Air` (free also) for automatically formatting our `R` code.

While some of you might have access to free trials or free programs for students that give you access to AI tokens for coding, some of you might have to pay for these service (about $20 USD a month for basic accounts: those should be enough for the course). Some options include [GitHub Copilot](https://github.com/features/copilot) (could be free through [GitHub Education](https://education.github.com/pack)) and [Posit AI](https://posit.co/products/ai) (has a 30 day free trial).
...
We will also make use of the `Positron` interactive development environment (IDE). `Positron` requires that `R` be installed, and so is an "add-on" to `R`. You can obtain `Positron` for free from the [Posit web site](https://positron.posit.co/download.html). It is also essential that you have the latest release of `Positron`. You can determine the version of `Positron` by looking at menu item `Positron > About Positron`. You should be using **`Positron` version 2026.08.2 or higher**.

Alternatively to `Positron`, you can use the older `RStudio IDE`. `RStudio IDE` also requires that `R` be installed first. You can obtain the `RStudio IDE` for free from the [Posit web site](https://posit.co/downloads). It is also essential that you have the latest release of `RStudio IDE`. You can determine the version of `RStudio IDE` by looking at menu item `Help > About RStudio`. You should be using **RStudio IDE version 2026.08.2 or higher**.
...
-   Project 0: end of week 2 (Sunday September 13, 11:59pm) (entirely optional and not graded but hopefully useful and fun)
-   Project 1: end of week 3 (Sunday September 20, 11:59pm)
-   Project 2: end of week 5 (Sunday October 4, 11:59pm)
-   Project 3: end of week 8 (Sunday October 25, 11:59pm)
...
Students requiring accommodations for disabilities should register with [Student Disability Service (SDS)](https://publichealth.jhu.edu/about/inclusion-diversity-anti-racism-and-equity-idare/student-disability-services). It is the responsibility of the student to register for accommodations with SDS. Accommodations take effect upon approval and apply to the remainder of the time for which a student is registered and enrolled at the Bloomberg School of Public Health. Once you request has been approved you will receive formal notification and the instructor will reach out to you via email. If you have questions about requesting accommodations, please contact `BSPH.dss@jhu.edu`.
...
You must install [R](https://cran.r-project.org) and [Positron](https://positron.posit.co/download.html) (or [RStudio IDE](https://posit.co/products/open-source/rstudio/?sid=1); both `Positron` and `RStudio IDE` are available from [Posit](https://posit.co/)) on your computing environment in order to complete this course. These are three **different** applications that must be installed separately before they can be used together:
...
6a.  Once your application has been approved, If you are using `Positron`, follow the instructions at <https://positron.posit.co/assistant> to enable "Positron assistant". Then under the "Positron Assistant" chat, click on "Add Anthropic as a Chat Provider", then select "GitHub Copilot" and click "Sign in".
...
6b.  If you are using `RStudio IDE` follow the instructions at <https://docs.posit.co/ide/user/ide/guide/tools/copilot.html> to enable `GitHub Copilot` in `RStudio IDE`. I have the "Index project files with GitHub Copilot" checkbox enabled on my preferences.
