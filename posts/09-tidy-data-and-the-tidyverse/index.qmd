...
3.  [`tidyr` cheat sheet from Posit](https://rstudio.github.io/cheatsheets/html/tidyr.html)
:::
...
-   [magrittr](https://cran.r-project.org/package=magrittr%22): defines the `%>%` operator for chaining functions together in a series of operations on data. You can also use the pipe operator from base R (`|>`) for most things, though the `magrittr` pipe `%>%` has some additional features that make it more flexible.
...
-   [tidyr cheat sheet from RStudio](https://rstudio.github.io/cheatsheets/html/tidyr.html)
:::
