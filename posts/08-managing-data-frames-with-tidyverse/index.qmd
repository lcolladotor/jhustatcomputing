...
1.  The `View()` function in `RStudio IDE` is incredibly helpful (although it's been enhanced in `Positron`!). The input to this function is the `data.frame` or tibble you would like to see.

Specifically, `View(chicago)` would provide you, the viewer, with a scrollable view (in a new tab) of the complete dataset. In `Positron` you also get small histograms for numerical variables as well as little bars showing what percent of the observations are missing for each variable. You can also click on a given numerical column and get the summary statistics for that column (equivalent of `summary(chicago$column_name)`).
...
-   `%>%`: the "pipe" operator is used to connect multiple verb actions together into a pipeline. Note that the base R package now includes a pipe operator `|>` that has similar functionality.
...
The `dplyr` package can be installed from CRAN or from GitHub using the `remotes` package and the `install_github()` function. The GitHub repository will usually contain the latest updates to the package and the development version.
