...
-   <https://r4ds.had.co.nz/functions#environment>
...
Writing functions is a **core activity** of an R programmer. It represents the key step of the transition from a "user" to a "developer" who creates new functionality for R.
...
-   Functions can be passed as arguments to other functions.
    -   This is very handy for the various apply functions, like `lapply()` and `sapply()` from base R and `tidyverse` loop functions implemented in `purrr`.
-   Functions can be nested, so that you can define a function inside of another function.
:::

If you are familiar with a common coding language like C, these features might appear a bit strange. However, they are really important in R and can be useful for data analysis.
...
But often it is useful **if a function returns something** that perhaps can be fed into another section of code. As we've seen before, `dplyr` functions often return a modified version of the input data frame so you can keep piping the functions together.
...
In my team guidelines for writing papers, I recommend that all non-default arguments be specified in the methods section of papers. See <https://lcolladotor.github.io/bioc_team_ds/writing-papers.html#styling> for more details.
...
There is a **special argument in R known as the `...` argument** (the ellipsis argument), which indicates **a variable number of arguments** that are usually passed on to other functions.
...
See <https://lcolladotor.github.io/dots/dots.html> for some work I did in 2014 around the `...` argument that I described in this blog post <https://lcolladotor.github.io/2014/12/11/use-hidden-advanced-arguments-for-user-friendly-functions/>. Though `dots` is not actively maintained.
