...
-   Know how to efficiently work within `Positron` or `RStudio IDE` for efficient literate programming
-   Create a `quarto` or `R Markdown` document
:::
...
Because this is lecture is built in using these tools ^[Technically it's a `quarto` file with a `.qmd` file extention. But `quarto` is very similar to `R markdown`, aka `.qmd` files are similar to `.Rmd` files.], let's demonstrate how this work. I am going to change `eval=FALSE` to `eval=TRUE`.
...
The source file for this lecture is [`posts/05-literate-programming/index.qmd`](https://github.com/lcolladotor/jhustatcomputing/blob/main/posts/05-literate-programming/index.qmd). There, we can see another `R` code chunk syntax.
...
The results are identical on the `html` file, so we can't tell which syntax was used to make the code chunk above. In any case, you can see that the code was executed and the plot was made.
...
-   <https://github.com/daianna-glez/daianna_blog> and <https://daianna-glez.github.io/daianna_blog/>
...
-   <https://github.com/gabrielramirezv/gabrielramirezv.github.io> and <https://gabrielramirezv.github.io/>
...
-   <https://github.com/lcolladotor/StatComputing_ProjectDemo> and <https://lcolladotor.github.io/StatComputing_ProjectDemo/>

# Tips and tricks in `Positron`

Here are shortcuts and tips on efficiently using `Positron` to improve how you write code. See <https://positron.posit.co/keyboard-shortcuts.html> for the keyboard shortcuts in `Positron`. The *run code* shortcut is the same in `Positron` and `RStudio IDE`:
...
If you are used to `RStudio IDE` keybindings, check also <https://positron.posit.co/migrate-rstudio-keybindings.html>. For example, the *insert a comment* shortcut is one of them:
...
In `markdown` (applies to both `R Markdown` and `quarto`), when creating an ordered list such as this one:
...
### New code chunk in `quarto` or `R Markdown`

When editing `quarto` or `R Markdown` documents, you will need to insert a new `R` code chunk many times. The following shortcuts will make your life easier:
...
## Tips and tricks in `R Markdown` in `RStudio IDE`
