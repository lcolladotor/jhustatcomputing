...
OK so our current location is in the reading and writing lectures sub-folder of the `jhustatcomputing2023` course repository. Let's try using the `here` package.
...
Now we see that using the `here::here()` function is a *relative* path (relative to the `.Rproj` file in our `jhustatcomputing2023` repository. We also see there is are two `.csv` files in the `data` folder. We will learn how to read those files into R in the next section.
