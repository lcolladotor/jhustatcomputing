...
*`GitHub`* is a hosting service on internet for `git`-aware folders and projects
   ...
   Technically *`GitHub`* is distinct from *`Git`*. However, *`GitHub`* is in some sense the interface and *`Git`* the underlying engine (a bit like *`RStudio IDE`* and *`R`*).
...
-   `RStudio IDE` also has [Git/GitHub integration](https://happygitwithr.com/rstudio-git-github.html). Of course this only works for `R` project `GitHub` integration.
...
We will mostly be using Git commands through the `RStudio IDE` Git panel or `Positron` Git Source Control section. This panel will show up by default if `RStudio IDE` / `Positron` recognizes that you have installed Git already.
...
2.  Log in to your GitHub account and access <https://github.com/lcolladotor/jhustatcomputing/issues/10>
