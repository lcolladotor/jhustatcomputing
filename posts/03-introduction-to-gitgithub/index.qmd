...
`GitHub` is a hosting service on internet for `git`-aware folders and projects
   ...
   Technically `GitHub` is distinct from `git`. However, `GitHub` is in some sense the interface and `git` the underlying engine (a bit like `Positron` or `RStudio IDE` and `R`).
...
To learn a bit more about `git`/`GitHub` and why you might want to use it, [read this article by Jenny Bryan](https://doi.org/10.1080/00031305.2017.1399928) (or the [preprint](https://peerj.com/preprints/3159/)).

**Note her explanation of what's special with the `README.md` file on `GitHub`.**

### What to (not) do

*Note*: In the following, I will use `GitHub` to refer to the hosting service and `git` to refer to the version control system.

`git` + **`GitHub` is ideal if** you have a project with a fair number of files, most of those files are text files (such as code, $LaTeX$, (R)`markdown`, `quarto`, etc.) and different people work on different parts of the project.
...
`git` and `GitHub` is fundamentally **based on commands you type into the command line**. Lots of online resources show you how to use the command line. This is the most powerful, and I use it frequently to interact with `git`/`GitHub`. However, many folks find this the most confusing way to use `git`/`GitHub`. Alternatively, there are graphical interfaces.

-   [GitHub itself](https://desktop.github.com/) provides a graphical interface with basic functionality.
-   `Positron` works very well with `Git` and `GitHub` as documented at <https://positron.posit.co/git.html>. It has several more advanced options, as `Positron` is based on `Visual Studio Code` (VS Code) and VS Code already has an in-depth integration with `Git` and `GitHub` as noted at <https://code.visualstudio.com/docs/sourcecontrol/intro-to-git>.
-   `RStudio IDE` also has [Git/GitHub integration](https://happygitwithr.com/rstudio-git-github.html). This only works for `R` project `GitHub` integration.
...
**Note**: As a student, you can (and should) upgrade to the Pro version of `GitHub` for free (i.e. access to unlimited private repositories is one benefit), see the [`GitHub` student developer pack](https://education.github.com/pack) on how to do this.

We will mostly be using `git` commands through the `Positron` Git Source Control section or `RStudio IDE` Git panel. This panel will show up by default if `RStudio IDE` / `Positron` recognizes that you have installed `git` already.

# Getting Started

One of my favorite resources for getting started with `git`/`GitHub` is the *Happy Git with R* from [Jenny Bryan](https://bsky.app/profile/jennybryan.bsky.social):
...
It truly is one of the **best resources** out there for getting started with `git`/`GitHub`, especially with the integration to `RStudio IDE`. Therefore, at this point, I will encourage all of you to go read through the online book.
...
Alternatively, check the [git to know git: an 8 minute introduction](https://research.libd.org/rstatsclub/2018/04/17/git-to-know-git/) blog post by [Amy Peterson](https://amy-peterson.github.io/) ([LinkedIn](https://www.linkedin.com/in/amypeterson6/)).
...
2.  Log in to your GitHub account and access <https://github.com/lcolladotor/jhustatcomputing/issues/12>

3.  Copy paste the following R code `stop("This R error is weird")`.

4.  Type `reprex::reprex()` in your R console.

5.  Paste the output into <https://github.com/lcolladotor/jhustatcomputing/issues/12> and click on the "comment" green button.
...
[Here's a nice comment](https://github.com/tidyverse/tidy-dev-day/issues/25#issuecomment-2233564971) from [Hadley Wickham](https://bsky.app/profile/hadley.nz) about an issue I made:
