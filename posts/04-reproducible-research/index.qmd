...
-   The directory where you have an RStudio project file (`*.Rproj`). For RStudio projects with a `git` repository, this is typically the same directory. Note that this file is not used by `Positron` which makes it even more important to use `git` on your project folders (directories). See <https://positron.posit.co/migrate-rstudio-rproj.html> for more details.
