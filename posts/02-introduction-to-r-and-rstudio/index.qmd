...
-   Install `R` and `Positron` (alternatively `RStudio IDE`) on your computer.
...
-   The `R` Community is very dynamic, helpful and welcoming.
    -   Check out the [#RStats](https://bsky.app/search?q=%23RStats) or [#rtistry](https://bsky.app/search?q=%23rtistry) on BlueSky, [TidyTuesday](https://www.tidytuesday.com) podcast and community activity in the [R4DS Online Learning Community](https://www.rfordatasci.com), and [r/rstats](https://www.reddit.com/r/rstats/) subreddit.
...
Today `R` runs on almost any standard computing platform and operating system. Its open source nature means that anyone is free to adapt the software to whatever platform they choose. Indeed, `R` has been reported to be running on modern tablets, phones, PDAs, internet browsers, and game consoles.
...
Code so you can reproduce these trends for yourself: gist.github.com/JEFworks/1f0...`<br><br><a href="https://bsky.app/profile/did:plc:lovemlyb7wlsonhsija6z3pc/post/3lupn6iom4s2x?ref_src=embed"`{=html}\[image or embed\]</a>

</p>

— Dr. Jean Fan (`<a href="https://bsky.app/profile/did:plc:lovemlyb7wlsonhsija6z3pc?ref_src=embed"`{=html}@jef.works</a>) `<a href="https://bsky.app/profile/did:plc:lovemlyb7wlsonhsija6z3pc/post/3lupn6iom4s2x?ref_src=embed"`{=html}July 24, 2025 at 9:27 AM</a>
...
-   There are over `r nrow(available.packages())` packages on CRAN that have been developed by users and programmers around the world.

-   There are also `r length(BiocManager::available()) - nrow(available.packages())` packages associated with the [Bioconductor project](http://bioconductor.org). That's a total of `r length(BiocManager::available())` packages available from CRAN and Bioconductor combined.
...
1.  How many R packages are on GitHub today?
:::
...
# Using `R` and `Positron` (alternatively `RStudio`)

> If `R` is the engine and bare bones of your car, then `Positron` (alternatively `RStudio IDE`) is like the rest of the car. The engine is super critical part of your car. But in order to make things properly functional, you need to have a steering wheel, comfy seats, a radio, rear and side view mirrors, storage, and seatbelts. --- *Nicholas Tierney*

\[[Source](https://qmd4sci.njtierney.com)\]

## `Positron IDE`

[![Interface of the Positron IDE showing Activity Bar, Primary Side Bar, Editor, Panel, and Secondary Side Bar](https://positron.posit.co/images/user-interface-for-rstudio-migration.jpeg)](https://positron.posit.co/layout.html)
...
_Source: [Positron IDE Layout](https://positron.posit.co/layout.html)._

You might also find the [quick tour of `Positron`](https://posit.co/blog/a-quick-tour-of-positron) very useful.

## `RStudio IDE`

The `RStudio IDE` layout has the following features:
...
[![A screenshot of the RStudio integrated developer environment (IDE) -- aka the working environment](https://github.com/njtierney/qmd4sci/raw/main/figs/rstudio-screenshot.jpeg)](https://qmd4sci.njtierney.com/)
...
### Installing `R` and `Positron` (alternatively `RStudio`)

-   If you have not already, [install `R` first](http://cran.r-project.org). If you already have `R` installed, make sure it is a fairly recent version, version 4.6.1 or newer. If yours is older than version 4.6.0, I suggest you update (install a new `R` version).
-   Once you have `R` installed, install the free version of [Positron](https://positron.posit.co/download.html). Again, make sure it's a recent version.
-   Alternatively, install the latest version of [RStudio IDE](https://posit.co/download/rstudio-desktop/).

::: callout-tip
Installing `R` and `RStudio IDE` should be fairly straightforward. However, a great set of detailed instructions is in Rafael Irizarry's `dsbook`

-   <https://rafalab.github.io/dsbook/installing-r-rstudio.html>
:::

If things don't work, ask for help in the Courseplus Discussion forum.
...
#### `Positron IDE` options

Check the [_How to get Air (auto-code formatter)_](https://lcolladotor.github.io/jhustatcomputing/syllabus.html#how-to-get-air-auto-code-formatter) section on the course Syllabus for the `Positron` options I recommend using.

#### `RStudio IDE` default options
...
To install an `R` package from CRAN, one can simply call the `install.packages()` function and pass the name of the package as an argument. For example, to install the `ggplot2` package from CRAN: open `Positron IDE` or `RStudio IDE`, go to the `R` prompt (the `>` symbol) in the lower-left corner and type
...
In `RStudio IDE`, you can also install (and update/remove) packages by clicking on the 'Packages' tab in the bottom right window.
...
### Getting started in `Positron` (or `RStudio IDE`)

While one can use `R` and do pretty much every task, including all the ones we cover in this class, without using `Positron IDE` or `RStudio IDE`, they are both very useful, have lots of features that make your `R` coding life easier and have both become pretty much the default integrated development environment (IDE) for `R`. Since `Positron IDE` and `RStudio IDE` have lots of features, it takes time to learn them. A good resource to learn more about `Positron IDE` or `RStudio IDE` are the [Posit Videos](https://posit.co/resources/videos/) collection.

::: callout-tip
For more information on setting up and getting started with `R`, `Positron IDE`, `RStudio IDE`, and `R` packages, read the Getting Started chapter in the `dsbook`:
...
-   [RStudio Conf 2019](https://resources.rstudio.com/resources/rstudioconf-2019/) talk by [Brooke Watson Madubuonwu](https://www.linkedin.com/in/brookemadu/) on "[R at the ACLU: Joining tables to to reunite families](https://posit.co/resources/videos/r-at-the-aclu-joining-tables-to-to-reunite-families/)"

-   [RStudio Conf 2020](https://resources.rstudio.com/resources/rstudioconf-2020/) talk by [María Teresa Ortíz Mancera](https://tereom.netlify.app/) on "[Mexican electoral quick count night with R](https://posit.co/resources/videos/mexican-electoral-quick-count-night-with-r-maria-ortiz-mancera/)"
