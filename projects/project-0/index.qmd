...
**Due date: September 13th at 11:59pm**

Using the tools we learned in the first week (e.g. `R`, `Positron` or `RStudio IDE`, and `git` + `GitHub`). Let's apply them in a small (but also comprehensive) exercise.
...
4.  The version of `Positron` that you have installed on your computer. To do this, click on `Positron` -\> `About Positron` and include the version number in your email.

* If you are using `RStudio IDE` instead of `Positron`, then include the version of `RStudio IDE` that you have installed on your computer. To do this start up `RStudio IDE` and in the R console window, run the following and again include the output in your email:

```{r rstudiov, eval = FALSE, echo = TRUE}
print(RStudio.Version()$version)
```

5.  If you have a `GitHub` username, please include the link to your GitHub profile in your email. For example, mine is <https://github.com/lcolladotor>. If you do not have a GitHub username, read <https://happygitwithr.com>, sign up for GitHub, and include your new username in your email to me.

::: callout-tip
### Tips about GitHub profiles

I realize that several of you just created your GitHub profiles for this class, or haven't used GitHub much before. This is noticeable by looking at your GitHub profiles, as several of them look a bit empty. Thus, you might want to edit it and link to your other work social media profiles, email, write some quick text about yourself, etc. See mine for example <https://github.com/lcolladotor>.

You could also make a special GitHub repo (named with your GitHub username) with a README.md file such as <https://github.com/lcolladotor/lcolladotor/blob/main/README.md> if you'd like to have a longer self description. In my case, I only have the image with my GitHub stats made with this line of code: <https://github.com/lcolladotor/lcolladotor/blob/main/README.md?plain=1#L1>.
:::

6. Apply to get free access to GitHub Copilot Pro as a student through the GitHub education pack: <https://education.github.com/pack>. Let me know if you were successful in your application.

7.  To make sure `git` is installed on your system, use the 'Terminal' (e.g. it's next to the R Console within `Positron` or `RStudio IDE`[^1]) (or whatever you use), run the following and include the output in your email:
...
Use `usethis::create_project()` or in `RStudio IDE` go to `File` -\> `New project` and create a project with the name `<YourGitHubUsername>.github.io`. That is a very specific name used by GitHub Pages <https://pages.github.com/>.

If you are using `Positron`, you can also do this through the `File` -\> `New Folder from Template...` -\> `R Project`.
...
`usethis::create_project()` will create a new `RStudio IDE` / `Positron` window for your website project.

### 2. Create a local Git repo for your website project

We will use utilities from `usethis` to link our project (our directoy with our website files) to GitHub. Assuming that you already have `git` installed in your computer (see <https://happygitwithr.com/install-git> for detailed instructions on how to install `git`), on your project you made on step 1 above type this on your R console: `usethis::use_git()`. This will run `git init` behind the scenes and setup a Git repository on your computer. On your R console you will get asked if it's OK to restart `RStudio IDE` or `Positron`. Say yes, as that will now enable the `git` panel in `RStudio IDE` / `Positron`, which will make your life easier later on.
...
Next, we can can link our local `git` repository to `GitHub`, which is a service that hosts `git` repositories. We will do this through `R` via the `usethis::use_github()` function. Since we configured `R` in step 4 to act for us when interacting with `GitHub`, we can now use this function on our website project.
...
-   <https://daianna-glez.github.io/> + <https://github.com/daianna-glez/daianna-glez.github.io>
...
You might also be interested in creating a `favicon` for your website. See <https://github.com/seankross/postcards/issues/65> for more details.

::: callout-note
### Link syntax common pitfalls

1. It's common for students to remove the `mailto:` part of an email link. If you do that, then your email link will not work, as your browser will interpret it as: "your website URL" + "your email", which is not a valid URL. For example, if your website is `https://lcolladotor.github.io/biostat776-intro-Leonardo-ColladoTorres/` and you want to email to `myemail@email.com`, then the link will be interpreted as `https://lcolladotor.github.io/biostat776-intro-Leonardo-ColladoTorres/myemail@email.com`, which is not a valid URL. Instead, you need to use `mailto:myemail@email.com` so that the link works correctly. This is an HTML syntax requirement, not something specific to R Markdown.

2. Frequently students forget to put `https://` before a URL. If you forget that part, then your browser will interpret it as: "your website URL" + "the URL you wanted to link to", which is not a valid URL. For example, if your website is `https://lcolladotor.github.io/biostat776-intro-Leonardo-ColladoTorres/` and you want to link to `www.google.com`, then the link will be interpreted as `https://lcolladotor.github.io/biostat776-intro-Leonardo-ColladoTorres/www.google.com`, which is not a valid URL. Instead, you need to use `https://www.google.com` so that the link works correctly.
:::
...
-   Feel free to share your website with the world. If you use Bluesky, use the `#postcards` hashtag and tag the author `@seankross.com` <https://bsky.app/profile/seankross.com>. Sean has a mostly inactive account on X (formerly Twitter) at <https://twitter.com/seankross>.

On the 5th Lecture, I will show as many websites as I can from CoursePlus during our class!
...
Once that's created, create the `R`/ and `data/` subdirectories. Inside `R/`, save your R scripts for every class. As for `data/`, copy the contents of <https://github.com/lcolladotor/jhustatcomputing/tree/main/data> into it. That way commands like `here("data", "chocolate.RDS")` from the class lectures will work the same way. See the tip about _Downloading course data files_ at the top of the [schedule](../../schedule.html#schedule-and-course-materials) for more details.
