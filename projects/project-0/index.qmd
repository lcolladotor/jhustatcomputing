...
7.  To make sure `git` is installed on your system, use the 'Terminal' (e.g. it's next to the R Console within RStudio[^1]) (or whatever you use), run the following and include the output in your email:
...
Use `usethis::create_project()` or in `RStudio IDE` go to `File` -\> `New project` and create a project with the name `<YourGitHubUsername>.github.io`. That is a very specific name used by GitHub Pages <https://pages.github.com/>. If you are using `Positron`, you can also do this through the `File` -\> `New Folder from Template...` -\> `R Project`.
